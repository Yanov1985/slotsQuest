...
// Получаем номер версии из аргументов командной строки
const versionNumber = process.argv[2] || '001';
const backupDir = path.join(__dirname, 'backups', 'db');
const backupFile = path.join(backupDir, `database_backup_v${versionNumber.padStart(3, '0')}.sql`);
...
console.log(`Создаю резервную копию базы данных версии ${versionNumber}...`);
...
try {
  ...
  console.log('Экспортирую данные через бекенд...');
  
  // Создаем скрипт экспорта в директории бекенда
  const exportScript = `
...
async function exportData() {
  try {
    ...
    let sql = '-- Резервная копия базы данных SlotQuest v${versionNumber}\\n';
    ...
    if (providers.length > 0) {
      ...
      for (const provider of providers) {
        ...
        sql += \`INSERT INTO providers (id, name, slug, logo_url, website_url, country, founded_year, description, is_active, is_recommended, created_at, updated_at) VALUES ('\${provider.id}', '\${name}', '\${slug}', \${logoUrl ? \`'\${logoUrl}'\` : 'NULL'}, \${websiteUrl ? \`'\${websiteUrl}'\` : 'NULL'}, \${country ? \`'\${country}'\` : 'NULL'}, \${provider.founded_year || 'NULL'}, \${description ? \`'\${description}'\` : 'NULL'}, \${provider.is_active || false}, \${provider.is_recommended || false}, '\${provider.created_at.toISOString()}', '\${provider.updated_at.toISOString()}');\\n\`;
      }
      sql += '\\n';
    }
    ...
    if (categories.length > 0) {
      ...
      for (const category of categories) {
        ...
        sql += \`INSERT INTO slot_categories (id, name, slug, description, created_at, updated_at) VALUES ('\${category.id}', '\${name}', '\${slug}', \${description ? \`'\${description}'\` : 'NULL'}, '\${category.created_at.toISOString()}', '\${category.updated_at.toISOString()}');\\n\`;
      }
      sql += '\\n';
    }
    ...
    if (themes.length > 0) {
      ...
      for (const theme of themes) {
        ...
        sql += \`INSERT INTO themes (id, name, slug, description, created_at, updated_at) VALUES ('\${theme.id}', '\${name}', '\${slug}', \${description ? \`'\${description}'\` : 'NULL'}, '\${theme.created_at.toISOString()}', '\${theme.updated_at.toISOString()}');\\n\`;
      }
      sql += '\\n';
    }
    ...
    // Сохраняем файл
    const outputFile = '${backupFile.replace(/\\/g, '\\\\')}';
...
`;
  ...
  const tempScript = path.join(backendDir, 'temp-export.js');
  fs.writeFileSync(tempScript, exportScript);
  
  // Выполняем скрипт
  execSync(`node temp-export.js`, { 
    cwd: backendDir,
    stdio: 'inherit'
  });
