                      ...
                      <div v-if="isTurboSectionVisible('infoPopupSection', showInfoPopupSection)" class="space-y-6 animate-fade-in">
                        ...
                        <div class="bg-[#1B1E26] rounded-lg p-4 border border-[#353A4A]">
                          <label class="flex items-center gap-2 text-sm font-medium text-[#E5E7EB] mb-3">
                            <span class="text-blue-400">📖</span> Overview
                          </label>
                          <textarea
                            v-model="form.overview"
                            rows="4"
                            class="w-full px-3 sm:px-4 py-2 sm:py-3 bg-[#0D1117] border border-[#353A4A] rounded-lg text-[#E5E7EB] placeholder-[#6B7280] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-sm"
                            placeholder="Detailed slot overview... (Auto-generated by AI)"
                          ></textarea>
                        </div>
                        ...
                        <div class="bg-[#1B1E26] rounded-lg p-4 border border-[#353A4A]">
                          <label class="flex items-center gap-2 text-sm font-medium text-[#E5E7EB] mb-3">
                            <span class="text-orange-400">⚙️</span> Mechanics
                          </label>
                          <textarea
                            v-model="form.mechanics"
                            rows="4"
                            class="w-full px-3 sm:px-4 py-2 sm:py-3 bg-[#0D1117] border border-[#353A4A] rounded-lg text-[#E5E7EB] placeholder-[#6B7280] focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all text-sm"
                            placeholder="Slot mechanics... (Auto-generated by AI)"
                          ></textarea>
                        </div>
...
const loadSlot = async () => {
  ...
  try {
    ...
    // Map new generated SEO schema keys to admin panel expectations (info_ prefix)
    Object.keys(localizations.value).forEach(locale => {
      let loc = localizations.value[locale];
      // Skip metadata or non-objects
      if (locale === '_metadata' || typeof loc !== 'object' || !loc) return;
      
      if (loc['expert_verdict'] && !loc['info_expert_verdict']) loc['info_expert_verdict'] = loc['expert_verdict'];
      if (loc['pros'] && !loc['info_pros']) loc['info_pros'] = JSON.stringify(loc['pros']);
      if (loc['cons'] && !loc['info_cons']) loc['info_cons'] = JSON.stringify(loc['cons']);
      if (loc['faq'] && !loc['info_faq']) loc['info_faq'] = JSON.stringify(loc['faq']);
      if (loc['how_to_play'] && !loc['info_how_to_play']) loc['info_how_to_play'] = JSON.stringify(loc['how_to_play']);
      if (loc['reviews'] && !loc['info_reviews']) loc['info_reviews'] = JSON.stringify(loc['reviews']);
    });
    ...
    if (!localizations.value['en']) {
      ...
      // Hydrate form.value with fields exclusively available in EN localization 
      // (like dynamically added overview or mechanics not in base schema)
      const enLoc = localizations.value['en']
      Object.keys(form.value).forEach(key => {
        if (typeof form.value[key] === 'string' && !key.includes('url') && !key.includes('_id') && key !== 'slug') {
          if (enLoc[key] !== undefined && enLoc[key] !== '') {
            form.value[key] = enLoc[key]
          }
        }
      })
    }
...
const saveSlot = async () => {
  ...
  try {
    ...
    // ===== SEO Schema Resynchronization =====
    // Ensure that stringified legacy fields are parsed back into clean JSON structures 
    // expected by the frontend's new architecture across ALL locales.
    Object.values(localizations.value).forEach(loc => {
      try {
        if (loc.info_expert_verdict) loc.expert_verdict = loc.info_expert_verdict;
        if (loc.info_pros) loc.pros = JSON.parse(loc.info_pros);
        if (loc.info_cons) loc.cons = JSON.parse(loc.info_cons);
        if (loc.info_faq) loc.faq = JSON.parse(loc.info_faq);
        if (loc.info_how_to_play) loc.how_to_play = JSON.parse(loc.info_how_to_play);
        if (loc.info_reviews) loc.reviews = JSON.parse(loc.info_reviews);
      } catch (e) {
        console.error('Error parsing SEO JSON schemas for saving locale:', e);
      }
    });
