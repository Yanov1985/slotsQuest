        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 py-3 lg:h-20 lg:py-0">
          <div class="flex items-center gap-3 sm:gap-4 min-w-0">
            ...
            <h1 class="text-lg sm:text-2xl font-black text-white tracking-tight flex items-center gap-2 sm:gap-3 min-w-0">
              <Icon name="solar:gamepad-bold" class="text-[#00EDFF] w-6 h-6 sm:w-8 sm:h-8 shrink-0" />
              <span class="truncate">Каталог Слотов</span>
            </h1>
          ...
          <div class="flex items-center gap-2 sm:gap-3 w-full lg:w-auto">
            <!-- Refresh -->
            <button @click="refreshSlots" class="p-2.5 sm:p-3 bg-[#0B0E14] border border-[#353A4A] hover:border-[#00EDFF]/50 text-[#A0AABE] hover:text-[#00EDFF] rounded-xl transition-all duration-300 transform active:scale-95 hover:shadow-[0_0_15px_rgba(0,237,255,0.15)] flex items-center justify-center shrink-0" title="Обновить список">
              <Icon name="solar:refresh-circle-bold" class="w-5 h-5" />
            </button>

            <!-- Add Slot Button -->
            <button @click="showAddModal = true" class="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-[#DF0078] to-[#CD0F8B] hover:from-[#CD0F8B] hover:to-[#9D0B6B] text-white rounded-xl font-bold transition-all duration-300 transform hover:-translate-y-0.5 shadow-[0_0_20px_rgba(205,15,139,0.3)] min-w-0">
              <Icon name="solar:add-square-bold" class="w-5 h-5" />
              <span class="truncate">Добавить слот</span>
            </button>
          </div>
        </div>
      </div>
