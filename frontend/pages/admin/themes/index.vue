<template>
  ...
  >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <template v-if="loading">
          <div
            v-for="i in 6"
            :key="i"
            class="bg-[#161A21]/50 border border-[#353A4A] rounded-xl p-6 animate-pulse"
          >
        ...
        <template v-else-if="filteredThemes.length === 0">
          <div class="col-span-full text-center py-12">
            <Icon
              ...
              class="text-6xl text-[#FF6E48] mb-4"
            />
        ...
        <template v-else>
          <div
            ...
            :key="theme.id"
            class="bg-[#161A21]/50 border border-[#353A4A] rounded-xl p-6 hover:border-[#FF6E48]/40 transition-all group"
          >
            <div class="flex items-center space-x-4 mb-4">
              <div
                class="w-16 h-16 bg-[#1B1E26]/50 rounded-lg flex items-center justify-center border"
                :style="{ borderColor: theme.color || '#FF6E48' }"
              >
                <Icon
                  v-if="isIconify(theme.icon)"
                  :name="theme.icon"
                  class="text-2xl"
                />
              ...
              <div class="flex-1">
                <h3
                  class="text-lg font-semibold text-white group-hover:text-[#FF6E48] transition-colors"
                >
                  {{ theme.name }}
                </h3>
            ...
            <div class="space-y-3 mb-4">
              <div class="flex items-center justify-between text-sm">
                ...
                <span class="text-[#00EDFF] font-medium">{{
                  theme.slots_count || 0
                }}</span>
              </div>
              <div class="flex items-center justify-between text-sm">
            ...
            <div
              ...
              class="text-sm text-[#A0AABE] mb-4 line-clamp-2"
            >
              {{ theme.description }}
            </div>

            <div
              class="flex items-center justify-between pt-4 border-t border-[#353A4A]"
            >
              <div class="flex items-center space-x-2">
                <button
                  @click="editTheme(theme)"
                  class="p-2 bg-[#1B1E26]/60 hover:bg-[#1B1E26] border border-[#353A4A] rounded-lg transition-colors"
                  title="Редактировать"
                >
                ...
                <button
                  ...
                  class="p-2 bg-[#1B1E26]/60 hover:bg-[#1B1E26] border border-[#353A4A] rounded-lg transition-colors"
                  :title="theme.is_active ? 'Деактивировать' : 'Активировать'"
                >
                  <Icon
                    :name="
                      theme.is_active ? 'heroicons:pause' : 'heroicons:play'
                    "
                  />
                </button>
                <button
              ...
              <NuxtLink
                :to="`/admin/themes/${theme.id}/slots`"
                class="text-xs text-[#00EDFF] hover:text-[#01BFCF] transition-colors flex items-center space-x-1"
              >
        ...
        <div class="p-6">
          <div class="flex items-center justify-between mb-6">
            ...
            >
              ...
              />
              <span>{{
                showAddModal ? 'Добавить тему' : 'Редактировать тему'
              }}</span>
            </h2>
            <button
              @click="closeModal"
              class="text-[#A0AABE] hover:text-white transition-colors"
            >
    ...
    >
      <div
        class="bg-[#161A21] border border-red-500/30 rounded-xl max-w-md w-full"
      >
        <div class="p-6">
          <div class="text-center">
            ...
            <p class="text-[#E5E7EB] mb-6">
              ...
              <strong>"{{ themeToDelete?.name }}"</strong>? Это действие нельзя
              отменить.
            </p>
...
const activeThemes = computed(
  () => themes.value.filter((theme) => theme.is_active).length,
)
const themesWithSlots = computed(
  () => themes.value.filter((theme) => (theme.slots_count || 0) > 0).length,
)
const popularThemes = computed(
  () => themes.value.filter((theme) => theme.is_popular).length,
)
