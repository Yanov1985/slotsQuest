...
/**
 * 🔄 ВОССТАНОВЛЕНИЕ ДАННЫХ В SQLITE (ДЛЯ ТЕСТИРОВАНИЯ)
 * Использует SQLite для проверки восстановления данных
 */
...
// Создаем Prisma клиент для SQLite
...
async function initSqliteDb() {
  ...
  // Генерируем Prisma клиент для SQLite
  ...
  try {
    // Копируем схему
    const schemaPath = path.join(__dirname, 'schema-sqlite.prisma');
    const tempSchemaPath = path.join(__dirname, 'prisma', 'schema-temp.prisma');
    
    if (fs.existsSync(schemaPath)) {
      fs.copyFileSync(schemaPath, tempSchemaPath);
      console.log('✅ Схема SQLite скопирована');
    }
    
    // Генерируем клиент
    await execAsync('npx prisma generate --schema=./schema-sqlite.prisma');
    console.log('✅ Prisma клиент сгенерирован для SQLite');
    
    // Применяем миграции
    await execAsync('npx prisma db push --schema=./schema-sqlite.prisma');
    console.log('✅ Схема применена к SQLite');
...
async function clearTables() {
  ...
  try {
    ...
    console.warn('⚠️ Ошибка при очистке таблиц:', error.message);
  }
}
...
async function main() {
  try {
    ...
    console.log('🗄️ Инициализация SQLite...');
    await initSqliteDb();
    ...
    console.log('');
    console.log('🎉 ВОССТАНОВЛЕНИЕ В SQLITE ЗАВЕРШЕНО УСПЕШНО!');
    console.log('Данные успешно восстановлены в локальную SQLite базу данных');
    console.log('');
    console.log('💡 Теперь нужно решить проблему с подключением к Supabase');
    console.log('   и перенести данные в основную базу данных');
    
    return stats;
    
  } catch (error) {
    console.error('❌ Ошибка при восстановлении данных в SQLite:', error);
    throw error;
  } finally {
